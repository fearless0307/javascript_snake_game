...
function storeScore() {
    ...
    var storage = localStorage.getItem('Score'); // Get local storage
...
function draw(){
    ...
    // game over
    if(snakeX < box || snakeX > 17 * box || snakeY < 3*box || snakeY > 17*box || collision(newHead, snake) || collisionWall(newHead)){
        clearInterval(game);
        dead.play();
        reset();
    }else{
